...
def analyze_graph(a, b, c, k):
    print(f"k={k}")
    sys.stdout.flush()
    print()
    sys.stdout.flush()
    p1 = graphs.PathGraph(a)
    p2 = graphs.PathGraph(b)
    p3 = graphs.PathGraph(c)
    P = p1.cartesian_product(p2)
    g = P.cartesian_product(p3)
    show(g)
...
c_value = 4
k_value = 2
analyze_graph(a_value, b_value, c_value, k_value)
